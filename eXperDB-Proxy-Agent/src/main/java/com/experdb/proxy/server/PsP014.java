 ...
 * </pre>
 */
public class PsP014 extends SocketCtl{
	...
	public PsP014(Socket socket, BufferedInputStream is, BufferedOutputStream	os) {
		this.client = socket;
		this.is = is;
		this.os = os;
	}
	...
	public void execute(String strDxExCode, JSONObject jObj) throws Exception {
		socketLogger.info("PsP014.execute : " + strDxExCode);
		byte[] sendBuff = null;
		...
		try {
			...
			outputObj = fromJettisonToSimple(service.setConfScale(fromSimpleToJettison(jObj)));
			sendBuff = outputObj.toString().getBytes();
			send(4, sendBuff);
		} catch (Exception e) {
			errLogger.error("PsP014 {} ", e.toString());
			
			outputObj.put(ProtocolID.DX_EX_CODE, TranCodeType.PsP014);
