...
public class SecurityPolicyServiceCall {
	 ...
	 * @throws Exception
	 */
	public void insertProfileProtection(String restIp, int restPort, String strTocken, ProfileProtection param1, List param2, List param3) throws Exception {
		...
		if(resultCode.equals(SystemCode.ResultCode.SUCCESS)) {
			...
			if(arrProfileAclSpec.size()>0) {
				for(int i=0; i<arrProfileAclSpec.size(); i++) {
					...
					String startDateTime = profileAclSpec.getStartDateTime();
					System.out.println(startDateTime.substring(0, 1) + ":" + startDateTime.substring(2, 4) + ":"+ startDateTime.substring(4, 6));
					
					String startTime = profileAclSpec.getStartTime();
					System.out.println(startTime.substring(0, 1) + ":" + startTime.substring(2,3));
					...
					jsonObj.put("startDateTime", profileAclSpec.getStartDateTime());
					jsonObj.put("endDateTime", profileAclSpec.getEndDateTime());
					jsonObj.put("startTime", profileAclSpec.getStartTime());
					jsonObj.put("endTime", profileAclSpec.getEndTime());
					jsonObj.put("workDay", profileAclSpec.getWorkDay());
		...
		if(resultCode.equals("0000000000")) {
			...
			if(totalListCount > 0) {
				for(int i=0; i<list.size(); i++) {
					...
					jsonObj.put("getBinName", cryptoKeySymmetric.getBinName());
