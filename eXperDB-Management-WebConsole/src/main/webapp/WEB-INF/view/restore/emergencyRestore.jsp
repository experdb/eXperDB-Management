...
<script type="text/javascript">
	$(window.document).ready(function() {
		...
		//validate
	    $("#restoreEmergencyRegForm").validate({
 	        rules: {
 	        	restore_nm: {
					required: true
				},
	...
	function fn_storage_path_set() {
		...
		if (asis_flag == "0") {
			...
			$("#restore_dir", "#restoreEmergencyRegForm").val("");
		}
	}
	...
	function fn_new_storage_check() {
		var new_storage = $("#restore_dir", "#restoreEmergencyRegForm").val();

		if(new_storage == ""){
			showSwalIcon('<spring:message code="restore.msg06" />', '<spring:message code="common.close" />', '', 'warning');
			return;
		}
				...
				if (data != null) {
					if(data.result.ERR_CODE == ""){
						if(data.result.RESULT_DATA.IS_DIRECTORY == 0){
							...
							$("#file_check_path", "#restoreEmergencyRegForm").val("Y");
							
/* 							$("#dtb_pth").val(new_storage + "${pgdata}");
							$("#svrlog_pth").val(new_storage + "${srvlog}"); */
							$("#dtb_pth").val(new_storage);
							$("#svrlog_pth").val(new_storage);
						}else{
	...
	function fn_clean() {
		$("#restore_dir", "#restoreEmergencyRegForm").val("");
		$("#dtb_pth", "#restoreEmergencyRegForm").val("${pgdata}");
		$("#svrlog_pth", "#restoreEmergencyRegForm").val("${srvlog}");
	}
	...
	function fn_restoreNm_check() {
		if (nvlPrmSet($("#restore_nm", "#restoreEmergencyRegForm").val(), "") == "") {
			showSwalIcon('<spring:message code="restore.msg01" />', '<spring:message code="common.close" />', '', 'warning');
			return;
		}
		
		//msg 초기화
		$("#restorenm_check_alert", "#restoreEmergencyRegForm").html('');
		$("#restorenm_check_alert", "#restoreEmergencyRegForm").hide();

		$.ajax({
			url : '/restore_nmCheck.do',
			type : 'post',
			data : {
				restore_nm : $("#restore_nm", "#restoreEmergencyRegForm").val()
			},
			beforeSend: function(xhr) {
				xhr.setRequestHeader("AJAX", true);
			},
			...
			success : function(result) {
				if (result == "true") {
					...
					$('#restore_nmChk', '#restoreEmergencyRegForm').val("success");
				} else {
					showSwalIcon('<spring:message code="restore.msg222" />', '<spring:message code="common.close" />', '', 'error');
					$('#restore_nmChk', '#restoreEmergencyRegForm').val("fail");
				}
			}
		});
	}
	...
	function fn_restore_start() {
		$("#restoreEmergencyRegForm").submit();
	}
	...
	function fn_restore_validate() {
		if(nvlPrmSet($("#restore_nmChk", "#restoreEmergencyRegForm").val(), "") == "" || nvlPrmSet($("#restore_nmChk", "#restoreEmergencyRegForm").val(), "") == "fail") {
			$("#restorenm_check_alert", "#restoreEmergencyRegForm").html('<spring:message code="restore.msg02"/>');
			$("#restorenm_check_alert", "#restoreEmergencyRegForm").show();
	...
	function fn_execute() {
		...
		$.ajax({
			...
			data : {
				...
				svrlog_pth : $("#svrlog_pth", "#restoreEmergencyRegForm").val(),
				bck_pth : $("#bck_pth", "#restoreEmergencyRegForm").val(),
				restore_cndt : 1,
				restore_flag : 0,
				restore_nm : $("#restore_nm", "#restoreEmergencyRegForm").val(),
				restore_exp : $("#restore_exp", "#restoreEmergencyRegForm").val()
			},
			dataType : "json",
			type : "post",
			beforeSend : function(xhr) {
			...
			success : function(result) {
				if (result != null) {
					if (result == "S") {
						showSwalIconRst('<spring:message code="restore.msg220" />', '<spring:message code="common.close" />', '', 'warning', 'rman_restore');
					} else {
						showSwalIcon('<spring:message code="message.msg32" />', '<spring:message code="common.close" />', '', 'error');
						return;
					}
	...
	function fn_restoreLogCall() {
		$.ajax({
			...
			success : function(result) {
				$("#exelog", "#restoreEmergencyRegForm").append(result.strResultData);
			},
			beforeSend : function(xhr) {
				xhr.setRequestHeader("AJAX", true);
			},
						...
						<div class="card" style="margin-bottom:0px;">
								...
								<div class="row">
									<div class="col-5" style="padding-top:3px;">
										<h6 class="mb-0">
											<a data-toggle="collapse" href="#page_header_sub" aria-expanded="false" aria-controls="page_header_sub" onclick="fn_profileChk('titleText')">
												...
												<span class="menu-title"><spring:message code="restore.Emergency_Recovery"/></span>
									...
									<div class="col-7">
					 					<ol class="mb-0 breadcrumb_main justify-content-end bg-info" > 
											...
											<li class="breadcrumb-item_main active" style="font-size: 0.875rem;" aria-current="page"><spring:message code="restore.Emergency_Recovery" /></li>
										</ol>
									</div>
								</div>
							</div>
							...
							<div id="page_header_sub" class="collapse" role="tabpanel" aria-labelledby="page_header_div" data-parent="#accordion">
								<div class="card-body">
									<div class="row">
										<div class="col-12">
											<p class="mb-0"><spring:message code="help.Emergency_Recovery" /></p>
										</div>
									</div>
								</div>
							</div>
		...
		<div class="col-12 stretch-card div-form-margin-table">
			<div class="card">
				<div class="card-body" style="min-height:698px; max-height:750px;">
					...
					<form class="cmxform" id="restoreEmergencyRegForm">
								...
								<div class="col-md-6 system-tlb-scroll" style="border:0px;max-height: 460px; overflow-x: hidden;  overflow-y: auto; ">
									<div class="card-body" style="border: 1px solid #adb5bd;">
										...
										<div class="form-group row div-form-margin-z" style="margin-top:-5px;" id="storage_view">
											...
											<div class="col-sm-5">
												<input type="text" class="form-control form-control-sm" id="restore_dir" name="restore_dir" value="" onblur="this.value=this.value.trim()" />
											</div>
										...
										<div class="form-group row div-form-margin-z" style="margin-top:-5px;">
											<label for="ipadr" class="col-sm-12 col-form-label pop-label-index" style="padding-top:7px;">
												<i class="item-icon fa fa-dot-circle-o"></i>
												Database Storage
											</label>
										</div>
										<div class="form-group row div-form-margin-z" style="margin-top:-20px;">
											<div class="col-sm-12">
