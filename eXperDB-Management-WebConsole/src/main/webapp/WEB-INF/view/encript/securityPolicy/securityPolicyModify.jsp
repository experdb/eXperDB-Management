...
<script>
	...
	function fn_save(){
		...
		for(var i = 0; i < datas2.length; i++){
			...
			policy["startDateTime"] = table2.rows().data()[i].startDateTime+"00:00:00";
			policy["endDateTime"] = table2.rows().data()[i].endDateTime+"00:00:00";
		...
		$.ajax({
			...
			data : {
				...
				nullEncryptYesNo : $("#nullEncryptYesNo").val(),
					...
					<table class="write">
						...
						<tbody>
							<tr>
								...
								<td>
									<input type="text" class="txt t2" name="profileName" id="profileName" value="${result.profileName}" readonly="readonly"/>
								</td>
							</tr>
							<tr>
								<th scope="row" class="ico_t1">보안정책설명</th>
								<td><textarea class="tbd1" name="profileNote" id="profileNote">${result.profileNote}</textarea></td>
							</tr>
						</tbody>
					</table>
				</div>
									...
									<tr>
										...
										<td>
											<c:forEach var="cryptoKey" items="${cryptoKey}">
												<c:if test="${cryptoKey.getBinUid eq ProfileCipherSpecData.binUid}" >${cryptoKey.getBinName}</c:if>	
											</c:forEach>
										</td>
										<td>
					...
					<table class="write">
						...
						<tbody>
							<tr>
								...
								<td>
									<div class="inp_rdo">
										<input name="defaultAccessAllowTrueFalse" id="rdo_2_1" type="radio" value="Y" ${result.defaultAccessAllowTrueFalse == 'true' ? 'checked="checked"' : ''} >
										<label for="rdo_2_1" style="margin-right: 15%;">예</label> 
										<input name="defaultAccessAllowTrueFalse" id="rdo_2_2" type="radio" value="N" ${result.defaultAccessAllowTrueFalse == 'false' ? 'checked="checked"' : ''}> 
							...
							<tr>
								...
								<td>
									<select class="select t3" id="denyResultTypeCode" name="denyResultTypeCode">
								...
								<td>
									<select class="select t3" id="dataTypeCode" name="dataTypeCode">
										<c:forEach var="dataTypeCode" items="${dataTypeCode}">
											<option value="<c:out value="${dataTypeCode.sysCode}"/>" ${result.dataTypeCode == dataTypeCode.sysCode ? 'selected="selected"' : ''}><c:out value="${dataTypeCode.sysCodeName}"/></option>
										</c:forEach> 
									</select>
								</td>
							</tr>
							<tr>
								<td colspan="2">
									<div class="inp_chk">
										<span> 
											<input type="checkbox" id="log_on_fail" name="log_on_fail" value="Y" ${result.log_on_fail == 'true' ? 'checked="checked"' : ''}/> 
											<label for="log_on_fail">실패 로그 기록</label>
										</span> 
										<span>
											<input type="checkbox" id="compress_audit_log" name="compress_audit_log" value="Y" ${result.compress_audit_log == 'true' ? 'checked="checked"' : ''}/> 
											<label for="compress_audit_log">로그압축</label>
										</span> 
										<span> 
											<input type="checkbox" id="preventDoubleYesNo" name="preventDoubleYesNo" value="Y" ${result.preventDoubleYesNo == 'Y' ? 'checked="checked"' : ''}/>
							...
							<tr>
								<td colspan="2">
									<div class="inp_chk">
										<span> 
											<input type="checkbox" id="log_on_success" name="log_on_success" value="Y" ${result.log_on_success == 'true' ? 'checked="checked"' : ''}/> 
											<label for="log_on_success">성공 로그 기록</label>
										</span> 
										<span> 
											<input type="checkbox" id="nullEncryptYesNo" name="nullEncryptYesNo" value="Y" ${result.nullEncryptYesNo == 'Y' ? 'checked="checked"' : ''}/>
