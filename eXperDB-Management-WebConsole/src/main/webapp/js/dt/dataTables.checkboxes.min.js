									...
									._fnApplyColumnDefs(
											...
											[ {
												...
												"render" : function(data, type,
														full, meta) {
													if (type === "display") {
//														data = '<div class="inp_chk"><input type="checkbox" class="dt-checkboxes"><label for="chk_1"></label></div>';
					...
					if (hasCheckboxes) {
						...
						$tableBody.on("click", "input.dt-checkboxes", function(
								e) {
							self.onClick(e, this);
						});
						if (hasCheckboxesSelectRow) {
							...
							$table.on("draw.dt select.dt deselect.dt",
									function() {
						...
						if (ctx.oFeatures.bStateSave) {
							dt.on("stateSaveParams.checkboxes", function(e,
									settings, data) {
				...
				updateCheckbox : function(type, selector, isSelected) {
													...
													var columns = $
															.grep(
																	...
																	function(
																			value) {
																		return (value != colIdx)
																				&& ctx.aoColumns[value].checkboxes.selectRow;
																	});
													if (columns.length) {
														$
																.merge(
					...
					if (nodes.length) {
						dt.$(nodes).find("input.dt-checkboxes").prop("checked",
								isSelected);
					}
				},
				...
				onSelect : function(e, type, indexes) {
					...
					if (type === "row") {
						self.updateCheckbox("row", indexes,
								(e.type === "select") ? true : false);
						self.updateData("row", indexes,
								(e.type === "select") ? true : false);
						self.updateSelectAll();
					}
				},
				onClickSelectAll : function(e, ctrl) {
					...
					var cells = dt
							.cells(
									...
									col,
									{
										page : ((ctx.aoColumns[col].checkboxes && ctx.aoColumns[col].checkboxes.selectAllPages) ? "all"
												: "current"),
										search : "applied"
									});
							...
							.every(
									function() {
										...
										var index = $
												.inArray(
														cellData,
														ctx.checkboxes.s.data[cellColIdx]);
										if (index !== -1) {
											if (!rows_seen
													.hasOwnProperty(cellRowIdx)) {
												self.updateCheckbox("cell",
														this.node(), true);
												self.updateSelect("cell", this
														.node(), true);
												if (ctx.aoColumns[cellColIdx].checkboxes
				...
				updateSelectAll : function() {
					...
					for (var colIdx = 0; colIdx < ctx.aoColumns.length; colIdx++) {
						if (ctx.aoColumns[colIdx].checkboxes
							...
							var $tableContainer = dt.table().container();
							var $checkboxes = dt.$(cells.nodes()).find(
									".dt-checkboxes");
							var $checkboxesChecked = dt.$(cells.nodes()).find(
				...
				showInfoSelected : function() {
					...
					if (colSelectRowIdx !== -1) {
						add("row",
								ctx.checkboxes.s.data[colSelectRowIdx].length);
			...
			Api.registerPlural("columns().checkboxes.select()",
					"column().checkboxes.select()", function(select) {
						...
						return this.iterator("column",
								function(ctx, colIdx) {
									if (ctx.checkboxes) {
										var selector = this.cells("tr", colIdx)
												.nodes();
										ctx.checkboxes.updateCheckbox("cell",
												selector, (select) ? true
														: false);
										ctx.checkboxes.updateData("cell",
												selector, (select) ? true
														: false);
										ctx.checkboxes.updateSelect("cell",
												selector, (select) ? true
														: false);
			...
			Api.registerPlural("cells().checkboxes.select()",
					"cell().checkboxes.select()", function(select) {
						if (typeof select === "undefined") {
							select = true;
						}
						return this.iterator("cell", function(ctx, rowIdx,
								colIdx) {
							if (ctx.checkboxes) {
								var selector = {
									row : rowIdx,
								...
								ctx.checkboxes.updateCheckbox("cell", selector,
										(select) ? true : false);
								ctx.checkboxes.updateData("cell", selector,
										(select) ? true : false);
								ctx.checkboxes.updateSelect("cell", selector,
										(select) ? true : false);
								ctx.checkboxes.updateSelectAll();
							}
						}, 1);
