 ...
 * 목록 으로 가기
 *********************************************************/
function fn_cmmnCodeDtlList(){
	var frm = document.insertCmmnDtlCode;
...
function fn_egov_regist_CmmnCode(){
				...
				if(fnCheckNotKorean(document.insertCmmnDtlCode.sys_cd.value)){
					window.alert("코드에 한글을 사용할 수 없습니다.");
					document.insertCmmnCode.grp_cd.select();
					return;
				}
				
				if(fnCheck(document.insertCmmnDtlCode.sys_cd.value)){
				...
				if(confirm("코드를 등록하시겠습니까?")){
					frm = document.insertCmmnDtlCode;
					frm.action = "/insertCmmnDtlCode.do";
					frm.submit();
				}
		}
