...
*      </pre>
*/
public class RunCommandExecNoWait extends Thread {
	...
	public RunCommandExecNoWait(){}
	
	public RunCommandExecNoWait(String cmd,  int intRestore_sn){
		this.CMD = cmd;
	...
	public void runExecRtn2(String cmd){
		...
		try{
			...
			if ( proc.exitValue() != 0 ) {
    			...
    			RmanRestoreVO failVo = new RmanRestoreVO();
    			failVo.setRESTORE_SN(intRestore_sn);
    			failVo.setRESTORE_CNDT(FAILED);
    			
    			// restore running end
    			service.updateRMAN_RESTORE_CNDT(failVo);
    			...
    			RmanRestoreVO successVo = new RmanRestoreVO();
    			successVo.setRESTORE_SN(intRestore_sn);
    			successVo.setRESTORE_CNDT(SUCCESS);
    			
    			// restore running end
    			service.updateRMAN_RESTORE_CNDT(successVo);
