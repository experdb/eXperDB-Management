...
public class DXTcontrolScale extends SocketCtl {
    ...
    public void start() {
		...
		SystemServiceImpl sysService = (SystemServiceImpl) context.getBean("SystemService");
...
/*        		try {
				...
				DbServerInfoVO resultMasterGbnVO = sysService.selectISMasterGbm(masterGbnVo);
				socketLogger.info("DXTcontrolScaleAwsExecute.strIpadr : " + strIpadr);
				socketLogger.info("DXTcontrolScaleAwsExecute.resultMasterGbnVO.getMASTER_GBN() : " + resultMasterGbnVO.getMASTER_GBN());

				//MASTER 일 경우만 실행
				if (resultMasterGbnVO != null) {
					if("M".equals(resultMasterGbnVO.getMASTER_GBN())) {
